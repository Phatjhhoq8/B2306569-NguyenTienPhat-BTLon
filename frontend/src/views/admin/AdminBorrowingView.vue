            ...
            <tr v-for="receipt in receipts" :key="receipt._id" class="hover:bg-slate-50/55 transition-colors">
              ...
              <td class="py-4 text-right space-x-2">
                ...
                <button 
                  ...
                  @click="payReceiptByAdmin(receipt)"
                  class="bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs py-1.5 px-3 rounded-xl transition-all shadow inline-block"
                >
                  Thanh toán
                </button>
...
const payReceiptByAdmin = async (receipt) => {
  const ok = await confirmModal.value.ask({
    title: 'Xác nhận thanh toán',
    message: `Bạn xác nhận độc giả đã thanh toán trực tiếp số tiền ${new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(receipt.tongTienThanhToan)} bằng tiền mặt tại quầy cho phiếu mượn ${receipt.maPhieu}?`,
  ...
  try {
    const res = await api.post(`/borrowing/receipts/${receipt._id}/pay`, {
      phuongThucThanhToan: 'TAI_QUAY'
    });
    if (res.success) {
      toast.show('Ghi nhận thanh toán tại quầy thành công!', 'success');
      fetchReceipts();
    }
  } catch (error) {
    toast.show(error.message || 'Lỗi khi thanh toán tại quầy', 'error');
  }
};
