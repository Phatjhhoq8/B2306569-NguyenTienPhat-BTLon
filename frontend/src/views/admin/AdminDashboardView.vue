<template>
  <div class="space-y-8">
    ...
    <div class="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm space-y-6">
      ...
      <div v-if="recentReceipts.length > 0" class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
          ...
          <tbody class="divide-y divide-slate-50 text-sm font-medium text-slate-700">
            <tr v-for="receipt in recentReceipts" :key="receipt._id" class="hover:bg-slate-50/55 transition-colors">
...
const stats = ref([
  { title: 'Tổng đầu sách', value: 0, icon: BookMarked, iconBg: 'bg-blue-600' },
...
const loadDashboardData = async () => {
  try {
    ...
    if (receiptsRes.success) {
      ...
      recentReceipts.value = allReceipts.slice(0, 5); // Lấy 5 phiếu mới nhất
      
      const activeLoans = allReceipts.filter(r => r.trangThai === 'DANG_MUON').length;
      const overdueLoans = allReceipts.filter(r => r.trangThai === 'QUA_HAN').length;
